...
realdata = get_datafiles(real_nprints_dir)
synthdata = get_datafiles(synth_nprints_dir)

results = defaultdict(dict)
for label, Y in tqdm(realdata['labels'].items()):
	print(f"evaluating models on {label}...\n")
	X_train, X_test, y_train, y_test = train_test_split(realdata['data'], Y, test_size=0.2, random_state=42)
	models = {
	...
	for name, model in tqdm(models.items()):
		print(f"training {name} on {label}...")
...
testdata = realdata
results = defaultdict(dict)
for label, (y_train, y_test) in tqdm(zip(synthdata['labels'].keys(), zip(synthdata['labels'].values(), testdata['labels'].values()))):
	print(f"evaluating models on {label}...\n")
	X_train, X_test = synthdata['data'], testdata['data']
	models = {
		'rf': RandomForestClassifier(n_estimators=100, random_state=42),
		'svc': SVC(kernel='linear', C=1.0, random_state=42),
		'xgb': XGBClassifier(objective='multi:softmax', num_class=len(np.unique(y_train)), use_label_encoder=False, seed=42)
	}
	for name, model in tqdm(models.items()):
		print(f"training {name} on {label}...")
		model.fit(X_train, y_train)
